...
class InterpretacionLenguajeNatural:
    def __init__(self):
        self.reset()  # Llama a reset para inicializar las variables
    ...
    def speech_to_text(self, from_file=True, filename=None):
        """
        Convierte audio a texto usando Google Speech-to-Text.
        
        Parámetros:
        - from_file (bool): Si es True, usa un archivo de audio. Si es False, usa el micrófono.
        - filename (str, opcional): Ruta del archivo de audio si from_file es True
        """
        recognizer = sr.Recognizer()
        
        try:
            if from_file:
                # Modo de archivo de audio (como en el método original)
                if not filename:
                ...
                # Eliminar el archivo temporal si se generó internamente
                if filename == "temp_audio.wav":
                    os.remove(filename)
            else:
                # Modo de micrófono (como en speech_to_text2())
                with sr.Microphone() as source:
                    print("Hable ahora")
                    audio = recognizer.listen(source)
            
            # Reconocimiento de voz común a ambos modos
    ...
    def solicitud_ingreso(self):
        ...
        print("texto: ",self.speech_to_text(from_file=False))
        ...
        self.text_to_speech("Solicitud registrada con exito ya puede ingresar su vehiculo")
    ...
    def solicitud_egreso(self):
        ...
        print(f"texto: {self.speech_to_text(from_file=False)}")
        datos_vehiculo = self.validar_datos_vehiculo()
    ...
    def text_to_speech(self, texto):
        ...
        tts.save(archivo)
        playsound(archivo)  # Reproduce el archivo usando playsound
        os.remove(archivo)  # Elimina el archivo después de reproducirlo
